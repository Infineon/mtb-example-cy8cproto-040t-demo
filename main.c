...
/* Active mode Scan time calculated in us ~= 95us */
#define ACTIVE_MODE_FRAME_SCAN_TIME     (95u)

/* Active mode Processing time in us ~= 92us with LEDs and Tuner disabled*/
#define ACTIVE_MODE_PROCESS_TIME        (92u)

/* ALR mode Scan time caluculated in us ~= 95us */
#define ALR_MODE_FRAME_SCAN_TIME        (95u)
...
void led_control()
{
    ...
    if(0u != Cy_CapSense_IsWidgetActive(CY_CAPSENSE_BUTTON0_WDGT_ID, &cy_capsense_context))
    {
        /* LED2 Turns ON when there is a touch detected on the CSX button*/
        Cy_GPIO_Write(CYBSP_USER_BTN_PORT, CYBSP_USER_BTN_NUM, CYBSP_LED_ON);
    }
    else if(0u != Cy_CapSense_IsWidgetActive(CY_CAPSENSE_BUTTON1_WDGT_ID, &cy_capsense_context))
    {
        /* LED2 Turns ON when there is a touch detected on the CSD button*/
        Cy_GPIO_Write(CYBSP_USER_BTN_PORT, CYBSP_USER_BTN_NUM, CYBSP_LED_ON);
    }
    else
    {
        /* LED Turns OFF when there is no touch detected */
